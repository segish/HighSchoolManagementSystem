<?php
$id11 = $_SESSION['uid'];
$sqlmain="SELECT * FROM `teacher` WHERE `id`='$id11'";
$run11=mysqli_query($con,$sqlmain);
$data11=mysqli_fetch_assoc($run11);
$fromname=$data11['fname'];
$sql22="SELECT *FROM message WHERE towhich = 'teacher' AND toname = '$fromname' ;";
    ...
    <body>
        ...
        <div class="newmessage" id="neemessage">
            ...
            <form method="post" id="form" name="myform" action="#">
                <div class="name-area">
                    <div class="input-control">
                        <select name="toname" id="toname">
                            ...
                            $sqlli="SELECT *FROM admin WHERE Fname != '$fromname';";
                        ...
                        $sqlli="SELECT *FROM teacher WHERE fname != '$fromname';";
                        $resultli=mysqli_query($con,$sqlli);
                        if($resultli){
                            while($rowli=mysqli_fetch_assoc($resultli)){
                                echo '
                        ...
                        $sqlli="SELECT *FROM parent WHERE fname != '$fromname';";
                        $resultli=mysqli_query($con,$sqlli);
                        if($resultli){
                            while($rowli=mysqli_fetch_assoc($resultli)){
                                echo '
                ...
                <div class="form-group mb-0">
                    <input class="btn btn-primary btn-block" name="submit1" id="submit" type="button" onclick="openpopup()" value="send"> 
                    <div class="popup" id="popup">
                        ...
                        <button name="send" type="submit" onclick="closepopup1()">OK</button>
                    </div>
                </div>
            </form> 
            <?php
...
$dbcon=mysqli_connect($servername,$username,$passcode,$db_name);
    ...
    $insertQ = "INSERT INTO `message`(`fromwhich`, `towhich`, `fromname`, `toname`, `message`) 
        VALUES ('teacher','$towhich','$fromname','$toname','$message')";
